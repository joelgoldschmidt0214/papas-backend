...
class UserProfileResponse(UserResponse):
    ...
    following_count: int = Field(
        default=0, ge=0, description="Number of users being followed"
    )
...
class TagResponse(BaseModel):
    ...
    posts_count: int = Field(
        default=0, ge=0, description="Number of posts with this tag"
    )
...
# 2025-08-22追加PostImageResponse（けいじゅ）
class PostImageResponse(BaseModel):
    """Response model for an image attached to a post"""

    image_url: str = Field(..., description="URL of the image")
    display_order: int = Field(..., description="Display order of the image")
...
class PostResponse(BaseModel):
    ...
    # 2025-08-22images追加、以下修正（けいじゅ）
    images: List[PostImageResponse] = Field(
        default=[], description="Associated images"
    )  # 画像リストを追加
    ...
    bookmarks_count: int = Field(
        default=0, ge=0, description="Number of bookmarks"
    )  # ブックマーク数を追加
    is_liked: bool = Field(
    ...
    is_bookmarked: bool = Field(
        default=False, description="Whether current user bookmarked this post"
    )
...
class SurveyResponse(BaseModel):
    ...
    question_text: Optional[str] = Field(
        None, max_length=1000, description="Survey question text"
    )
    ...
    target_audience: str = Field(
        default="all", max_length=100, description="Target audience"
    )
    created_at: datetime = Field(..., description="Survey creation timestamp")
    response_count: int = Field(
        default=0, ge=0, description="Number of responses received"
    )
